...
public class EditPushUpAction extends AbstractManagerAction {
	...
	protected ActionForward executeManagerAction(ActionMapping mapping,
		...
		try {
			...
			EditPushUpForm articleForm = (EditPushUpForm) form;
			IComponentFactory manager = (IComponentFactory) session
					.getServletContext().getAttribute(
							SatoriConstants.MANAGER_ATTR);
			IBItem item = manager.getItem();
			item.changeNotification(articleForm.getNotificationId(), articleForm.getText(), articleForm.getLangId(), articleForm.getTime());
