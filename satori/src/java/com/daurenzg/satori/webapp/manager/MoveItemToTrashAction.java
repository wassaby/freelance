...
public class MoveItemToTrashAction extends AbstractManagerAction {
	...
	protected ActionForward executeManagerAction(ActionMapping mapping,
		...
		try {
			...
			MoveItemToTrashForm deleteForm = (MoveItemToTrashForm) form;
			IComponentFactory manager = (IComponentFactory) session
					.getServletContext().getAttribute(
							SatoriConstants.MANAGER_ATTR);
			IBItem item = manager.getItem();
			item.moveItemToTrash(deleteForm.getItemId(), deleteForm.getLangId());
